<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-2xl p-6 w-full max-w-sm mx-4">
        <h2 class="text-xl font-bold text-red-600 mb-4">Konfirmasi Penghapusan Obat</h2>
        <p class="text-gray-700 mb-6">Anda yakin ingin menghapus data obat <strong id="drugNameToDelete"></strong>? Aksi ini tidak dapat dibatalkan. Pastikan obat ini tidak digunakan dalam resep aktif.</p>
...
<script>
    ...
    function openDeleteModal(drugId, drugName) {
        document.getElementById('drugNameToDelete').textContent = drugName;
        document.getElementById('deleteForm').action = '<?= base_url('admin/drugs/delete/') ?>' + drugId;
